...
import { RefreshCw, PartyPopper, Star, Square, Circle, Triangle, Heart, ToyBrick, Diamond } from 'lucide-react';
...
type ShapeMatchingGameProps = {
  gameRound: ReturnType<typeof useGameRound>;
};

const SHAPES = [
  { name: 'Star', icon: (props: any) => <Star {...props} /> },
  { name: 'Square', icon: (props: any) => <Square {...props} /> },
  { name: 'Circle', icon: (props: any) => <Circle {...props} /> },
  { name: 'Triangle', icon: (props: any) => <Triangle {...props} /> },
  { name: 'Heart', icon: (props: any) => <Heart {...props} /> },
  { name: 'Brick', icon: (props: any) => <ToyBrick {...props} /> },
  { name: 'Diamond', icon: (props: any) => <Diamond {...props} /> },
];


const generateCards = (pairs: number): Card[] => {
  const selectedShapes = [...SHAPES].sort(() => 0.5 - Math.random()).slice(0, pairs);
  const cardValues = [...selectedShapes, ...selectedShapes].map(s => s.name);
...
const ShapeIcon = ({ name, ...props }: { name: string, [key: string]: any }) => {
    const shape = SHAPES.find(s => s.name === name);
    if (!shape) return null;
    return shape.icon(props);
};

export function ShapeMatchingGame({ gameRound }: ShapeMatchingGameProps) {
        ...
        {cards.map((card, index) => (
          <div key={card.id} className="perspective-[1000px]">
            ...
            >
              ...
              {/* Card Front */}
              <div className={cn("absolute w-full h-full rounded-lg flex items-center justify-center shadow-lg",
                "backface-hidden rotate-y-180",
                card.isMatched ? "bg-green-400 text-white" : "bg-accent text-accent-foreground")}>
                <ShapeIcon name={card.value} className="w-16 h-16" fill="currentColor" />
              </div>
            </motion.div>
          </div>
        ))}
        ...
        {isGameWon && (
          ...
          >
            <div className="bg-white p-8 rounded-2xl shadow-2xl text-center">
              ...
              <p className="text-xl mt-2">Excellent shape matching skills!</p>
              <div className="mt-4">
